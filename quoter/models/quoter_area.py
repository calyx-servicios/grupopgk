...
class QuoterProfessionalArea(models.Model):
    ...
    @api.depends("hour_matrix_mode", "table_a_layout", "table_b_kind", "table_b_percent_mode")
    ...
    def write(self, vals):
        policy_fields = ("hour_matrix_mode", "table_a_layout", "table_b_kind", "table_b_percent_mode")
    ...
    def get_hours_matrix_preview_data(self):
        ...
        if combined and self.matrix_b_unify_role_values and self.matrix_b_hide_repeated_columns and rows_out:
            ...
            for row, line in zip(rows_out, lines_with_product):
                tmpl_id = line.product_tmpl_id.id
    ...
    def matrix_preview_write_cell(self, level_range_id, area_range_id, write_kind, value):
        ...
        if write_kind in hour_kinds:
            try:
                Policy.validate_hours_strictly_positive(val, _("Horas"))
            except ValidationError as e:
                raise UserError(str(e)) from e
        elif write_kind in ("matrix_b", "matrix_b_single"):
            try:
        ...
        if write_kind == "matrix_a":
            ...
            try:
                if self.matrix_a_unify_role_values:
                    target_lrs = LevelRange.search(
                        [
                            ("area_id", "=", self.id),
                            ("product_tmpl_id", "=", lr.product_tmpl_id.id),
                    ...
                    target_rows = target_lrs.mapped("matrix_a_ids").filtered(
                        lambda r: r.area_range_id.id == ar_id
                    )
