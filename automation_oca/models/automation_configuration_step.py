...
class AutomationConfigurationStep(models.Model):
    ...
    trigger_child_types = fields.Serialized(compute="_compute_trigger_child_types")
    trigger_type_data = fields.Serialized(compute="_compute_trigger_type_data")
    ...
    graph_data = fields.Serialized(compute="_compute_graph_data")
    ...
    def _get_field_translations(self, record, field_name):
        """Return stored translations for a field in installed languages.

        Odoo 15 does not expose the helper used in newer versions, so we
        explicitly read values by language context.
        """
        ...
        if "en_US" not in values:
            base_value = record.with_context(lang="en_US")[field_name]
            if base_value:
                values["en_US"] = base_value
        return values

    def _export_activity_values(self, activity_type):
        return {
            "name": self._get_field_translations(activity_type, "name"),
            "summary": self._get_field_translations(activity_type, "summary"),
    ...
    def _export_mail_template_values(self, mail_template):
        return {
            "name": self._get_field_translations(mail_template, "name"),
            "subject": self._get_field_translations(mail_template, "subject"),
            "body_html": self._get_field_translations(mail_template, "body_html"),
    ...
    def _export_server_action_values(self, server_action):
        return {
            "name": self._get_field_translations(server_action, "name"),
