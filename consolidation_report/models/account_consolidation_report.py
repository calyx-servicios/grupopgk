...
class AccountConsolidationReport(models.Model):
    ...
    def generate_consolidation_report_view(self):
        ...
        _logger.warning(f"[TIMING] validate_employees_departments: {_time.time()-_t:.2f}s"); _t = _time.time()
        ...
        self.delete_entries()
        _logger.warning(f"[TIMING] delete_entries: {_time.time()-_t:.2f}s"); _t = _time.time()
        ...
        _logger.warning(f"[TIMING] create_missing_analytic_lines: {_time.time()-_t:.2f}s"); _t = _time.time()

        # Calculo el monto total de las lineas de 'Gastos Indirectos'
        total_amount_cost = self.calculate_total_amount_cost()
        _logger.warning(f"[TIMING] calculate_total_amount_cost: {_time.time()-_t:.2f}s"); _t = _time.time()

        # Crear diccionario facturacion por proyecto
        total_sales_for_project = self.sales_by_project()
        _logger.warning(f"[TIMING] sales_by_project: {_time.time()-_t:.2f}s"); _t = _time.time()
        ...
        _logger.warning(f"[TIMING] calculate_percentage: {_time.time()-_t:.2f}s"); _t = _time.time()
        ...
        _logger.warning(f"[TIMING] create_analytic_lines_from_timesheets: {_time.time()-_t:.2f}s"); _t = _time.time()
        ...
        _logger.warning(f"[TIMING] UPDATE currency_id SQL: {_time.time()-_t:.2f}s"); _t = _time.time()
        ...
        _logger.warning(f"[TIMING] search analytic_lines ({len(analytic_lines)} registros): {_time.time()-_t:.2f}s"); _t = _time.time()
        ...
        comp_by_company_id = {cp.company_id.id: cp for cp in self.consolidation_period.consolidation_companies}
        ...
        for analytic_line in analytic_lines:
            ...
            consolidation_period = comp_by_company_id.get(_move_company_id) or comp_by_company_id.get(_src_company_id)
            ...
            if consolidation_period:
                ...
                elif _row.get('move_currency_id') and _row.get('move_currency_id') != _row.get('company_currency_id'):
                    is_historical = True
            ...
            consolidation_data_vals.append(
                {
                    ...
                    "sector_account_group": sector_account if sector_account else analytic_line.sector_account_id.id,
        ...
        _logger.warning(f"[TIMING] cost_to_project: {_time.time()-_t:.2f}s"); _t = _time.time()
        
        account_analytic_line_cost = self.analytic_line_cost(
            consolidation_data_vals_cost
        )
        _logger.warning(f"[TIMING] analytic_line_cost: {_time.time()-_t:.2f}s"); _t = _time.time()
        ...
        _logger.warning(f"[TIMING] create consolidation_data: {_time.time()-_t:.2f}s"); _t = _time.time()
    ...
    def _build_convert_amount_cache(self):
        ...
        comp_to_period = {cp.company_id.id: cp for cp in self.consolidation_period.consolidation_companies}
        return move_data, comp_to_period

    def _convert_amount(self, analytic_line, move_data=None, comp_to_period=None):
        if move_data is not None and comp_to_period is not None:
